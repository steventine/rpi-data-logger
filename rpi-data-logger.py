...
while True:
    ...
    with open(f'data/{curr_time:.0f}_data.csv', 'w', newline='') as f:
        ...
        while (curr_time < end_time):
            ...
            curr_time = time()
            camera.annotate_text = "Sec:{sec:.3f}".format(sec = curr_time) + " x: {x:.2f}, y: {y:.2f}, z: {z:.2f}".format(**raw)
            data_writer.writerow([round(curr_time, 3), round(raw["x"], 2), round(raw["y"], 2), round(raw["z"], 2)])
            if blink_timer >= 5:
